...
import { consumeResetToken, takeAuthQuota } from "@/lib/password-reset";
...
export async function POST(request: Request) {
  ...
  try {
    const { token, password, confirmPassword } = await readAuthBody(request);
    const invalid = "This reset link has expired or was already used. Request a new one.";
    if (typeof token !== "string" || !/^[a-f0-9]{64}$/.test(token)) return reply({ error: invalid }, 400);
    ...
    if (!(await takeAuthQuota("reset-global", "all", 100, 15 * 60 * 1000)) ||
        !(await takeAuthQuota("reset-token", token, 5, 15 * 60 * 1000))) {
      return reply({ error: "Too many attempts. Please wait 15 minutes or request a fresh link." }, 429);
    }
    const user = await consumeResetToken(token, password as string);
