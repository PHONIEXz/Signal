...
export default function UserMenu({ email }: { email?: string | null }) {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3">
      ...
      <button
        type="button"
        onClick={() => { try { for (const key of Object.keys(sessionStorage)) if (key.startsWith("signal-draft:")) sessionStorage.removeItem(key); } catch {} void signOut({ callbackUrl: "/login" }); }}
